...
void main() {
  runLiveTest('estimates repository live coverage', (context) async {
    ...
    await context.client.estimates.fireAction(created.id!, EstimateFireAction.markAsSent);
    final sent = await context.client.estimates.getEstimate(created.id!);
    expect(sent.status, EstimateStatus.sent);

    // 5. Search
    final search = await context.client.estimates.searchEstimates(query: 'Live test');
    expect(search.items.any((e) => e.id == created.id), isTrue);

    // 6. List
    final list = await context.client.estimates.getEstimates(status: EstimateStatus.sent);
